...
A workflow actor is an external program that drives an LLM session through a deterministic process. The LLM calls the workflow endpoint (`POST /api/sessions/{id}/workflow`) via the ouija skill; the program controls state, verification, and progression. Think of it as a process engine where the LLM is the task worker.
...
The pattern is the same as any tool call: the LLM calls deterministic code and gets a response. But this endpoint is custom per session — set at start time, managing its own state, controlling what the LLM does next. The LLM operates inside the steps; the workflow operates between them.
...
|---|---|---|---|
| 1. Skill description | The ouija skill description | Always in context | Tells the LLM the skill exists and how to call it |
...
- **How to interact** — call `workflow()`, follow what it says, call `workflow('init')` after any restart
...
This also keeps registration instructions small. A 5-line instruction set that says "call `workflow('init')`, do what it says, here's what NOT to do" consumes less context than a 50-line manual that duplicates what the workflow already controls.
...
**Registration** (called by the daemon at session start):
```json
// stdin
...
**Runtime** (called when the LLM calls `POST /api/sessions/{id}/workflow`):
```json
// stdin
...
- **Workflow endpoint**: Routes LLM `workflow()` calls to the executable, injects trusted `session_id`
...
**LLM -> Workflow** (synchronous, via REST API): The LLM calls the workflow endpoint, the daemon pipes to the executable, returns the response. This is request-response — the LLM initiates.
...
This replaces patterns where a coordinator LLM session reads a prompt, spawns workers via the REST API, polls for `done:` messages, and manages state through conversation context. The workflow script does all of this with deterministic code.
...
If a workflow returns the wrong response (e.g. reviewer instructions to a worker), the LLM will often "work around it" — sending messages directly instead of using workflow actions, or interpreting instructions creatively. This makes bugs hard to detect. Always verify the state file during testing to confirm sessions are in the expected states, not just that the end result looks correct.
...
A common multi-session pattern: worker spawns reviewer, then kills itself. But `POST /api/sessions` returns 202 (async) — the reviewer might not be alive yet. If the worker's worktree is cleaned up before the reviewer boots, the reviewer has no code to review.

Fix: poll for the new session before exiting. The retry-after pattern works here — `handle_complete` returns "call again to confirm reviewer" until the reviewer appears in the session list.
...
### Don't tell the LLM to kill its own session
...
- Signal completion via the REST API (`done=true`) without self-destructing

### Synchronous spawning from a workflow call will timeout

`POST /api/sessions` is async (returns 202). If a workflow action tries to spawn a session and wait for it to be ready in the same call, it will hit the daemon's timeout. Always spawn-and-continue: spawn in one action, check readiness in the next. The retry-after pattern handles this naturally.

## Migrating from loop_next

`loop-next` is deprecated. Workflows provide the same iteration capability with added
structure: state management, verification criteria, effort budgets, and multi-session coordination.

### Before (loop_next)

A session started with a prompt and reminder that tell it to convert files in a loop, calling loop-next after each one and signaling done when finished.
...
if action == "init":
    if not js_files:
        json.dump({"message": "All files converted! Call send(done=true)."}, sys.stdout)
    else:
        json.dump({
            "message": f"Convert `{js_files[0]}` to TypeScript. Run tests. Then call workflow('done', {{file: '{js_files[0]}'}}).",
    ...
    json.dump({"message": f"Recorded. {remaining} files remain. Call workflow('init') for the next file."}, sys.stdout)

sys.exit(0)
```

Start the session with the workflow attached (via REST API or by asking the assistant to start a "migrator" session with `migrate-workflow.py`).
