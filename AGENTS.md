...
A trusted SessionStart may replace an exact pane owner without managed-launch credentials only for proven cross-backend pane reuse: the incoming complete backend identity and live project must match the pane, and the incoming backend must be the only backend observed in its process tree. The daemon parks the exact incumbent and registers the new owner in one persisted pure transition. Same-backend thread rotation still requires generation-bound managed proof. This lifecycle replacement creates a new row with ordinary registration freshness; it does not refresh the dormant owner's role or bulletin metadata.
...
- **Local control plane:** an exact public Local session ID supplied by trusted injected context or by the operator is authoritative for an explicit CLI `--from`. The CLI marks that path in `SenderContext`; the daemon still requires the ID to name an existing `Origin::Local` session and rejects positive pane, environment, or complete backend-identity evidence that resolves the caller to a different Local session. Missing, not-found, or incomplete backend evidence is absence of proof, not proof of a sibling. Never infer an ID from a project name or pass a raw backend-native ID as `--from`.
