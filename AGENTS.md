...
An existing-pane `SessionStart` may adopt a backend binding only through the complete managed-launch proof: backend identity, named launch ID, exact daemon-issued incarnation, and the matching one-time credential consumed atomically by the bind. The later uncredentialed path is read-only for an already matching backend pair. Pane, cwd, live process state, and `@ouija_incarnation` are reusable observations, so they never authorize first binding or rebinding; brand-new manual discovery instead creates the session and backend pair atomically.

The separate `recover-backend-identity` operator path is limited to an exact existing Local owner whose backend and backend-session fields are both blank. It requires explicit public-ID intent plus a complete adapter identity, then independently corroborates the unchanged pane, canonical project, exact incarnation/markers, backend process, lease absence, and identity uniqueness while holding the resource gates. Its pure blank-to-bound compare-and-swap and durable persistence consume replay authority. It is not a `SessionStart`, generic registration, incomplete-metadata repair, or credentialed managed-launch fallback.

OpenCode readiness may reuse that same blank-to-bound recovery rule for a standalone interactive backend the managed opencode serve does not know. With exact pane+cwd hints it may select only one already-registered blank Local owner for that live pane and canonical project, then must pass the same owner-marker, backend-process, lease, and uniqueness checks. This path never creates a session and never treats pane/cwd without a complete adapter identity and the recovery checks as binding authority.

Blank-to-bound heals only a row that never bound. A row that bound and then went stale is repaired by the same pane-owner replacement, never by a same-incarnation rebind: OpenCode readiness may rotate an exact single Local pane owner off a populated-but-wrong opencode binding through `ReplaceReusedPaneOwner`, which keeps the public name, parks the incumbent in dormancy, and allocates a newer incarnation so no delayed observation owned by the stale incarnation leaks onto the new conversation. Rotation additionally requires the owner's canonical project to match the readiness cwd, exact owner-marker acceptance, backend-process corroboration, lease absence, identity uniqueness, and positive staleness evidence. That evidence is provenance-scoped: the managed serve is authoritative only for a `StrongManaged` binding it created, so its 404 refutes only that binding; a weak or standalone binding lives in an instance the serve cannot see, so its absence there proves nothing and only the in-pane plugin's `session.created` + root-session attestation may authorize rotation. Serve-visible child conversations and unattested reports always fail closed. Every readiness refusal is recorded in the bounded ring behind `GET /api/backend-session/declines` and named in the response `outcome`/`error`.

A trusted SessionStart may replace an exact pane owner without managed-launch credentials when a new backend conversation is corroborated in the same still-live pane. Same-backend thread rotation and cross-backend reuse both keep the pane's public Ouija name while allocating a newer incarnation in one persisted pure transition. A separate pane marker remembers the last public name after SessionEnd; dormant history does not reserve names. Only live sessions and lifecycle leases occupy public names, and an explicit local rename may take any otherwise-free historical name. Backend identity remains resource evidence, not authority over the user's local public name.
...
### Security boundaries for sender identity

Local explicit sender claims and Nostr sender identity use separate trust paths:

- **Local control plane:** an exact public Local session ID supplied by trusted injected context or by the operator is authoritative for an explicit CLI `--from`. The CLI marks that path in `SenderContext`; the daemon still requires the ID to name an existing `Origin::Local` session. An exact target-pane match is authoritative over stale environment or backend evidence; a pane positively owned by a different Local session still rejects the claim. Outside that exact pane, positive environment or complete backend-identity evidence resolving to a sibling also rejects it. Missing, not-found, or incomplete backend evidence is absence of proof, not proof of a sibling. Never infer an ID from a project name or pass a raw backend-native ID as `--from`.
- **Nostr transport:** never trust the wire message's `from` string as identity. NIP-17 decryption supplies the authenticated peer npub as `sender_npub`; `IncomingWire` checks any claimed `daemon_id` against that transport identity, scopes remote-session lookup to `Origin::Remote(sender_npub)`, and prefixes unknown senders with that peer's known daemon name or npub. Remote ingress cannot set the local `trusted_local_claim` marker or produce an unqualified Local-looking sender.

Do not merge these paths or reuse local sender-claim validation for remote ingress. Local authority comes from the local control plane plus daemon state; remote authority comes from verified Nostr transport provenance.
...
- **Hook trust.** `install()` writes/merges `~/.codex/hooks.json` idempotently. Normal installs may require Codex's hook trust-review; tests use `--dangerously-bypass-hook-trust`. The per-launch `hooks.state` trust override must be passed as one inline table — Codex splits a `-c` key on `.` before honoring quotes, so a dotted path parses cleanly (even under `--strict-config`) yet silently fails to trust the hook. Verify trust with a read-only `hooks/list` app-server probe, never with parse acceptance. See `docs/codex-cli.md`.
