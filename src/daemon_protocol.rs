...
impl DaemonState {
    ...
    pub fn apply(&mut self, event: Event) -> Vec<Effect> {
        match event {
            ...
            Event::Send {
                ...
                expects_reply,
                responds_to,
            } => self.apply_send(&from, &to, &message, expects_reply, responds_to),
        }
    }
    ...
    fn apply_incoming_wire(
        ...
        match msg {
            WireMessage::SessionSend {
                ...
                msg_id,
                responds_to,
            } => self.apply_incoming_send(&from, &to, &message, expects_reply, msg_id, responds_to),
    ...
    fn apply_incoming_send(
        ...
        // Clear pending reply if responds_to is set
    ...
    fn apply_send(
        ...
        match &session.origin {
            Origin::Local => {
                if let Some(ref pane) = session.pane {
                    let formatted =
                        format_session_message(from, message, expects_reply, msg_id, responds_to);
                    effects.push(Effect::InjectMessage {
                        pane: pane.clone(),
                        message: formatted,
                        vim_mode: session.metadata.vim_mode,
...
mod tests {
    ...
    fn format_message_xml_no_reply() {
        let msg = format_session_message("ouija", "hello", false, 42, None);
        assert_eq!(msg, r#"<msg from="ouija" id="42">hello</msg>"#);
    }

    #[test]
    fn format_message_xml_expects_reply() {
        let msg = format_session_message("ouija", "do this", true, 47, None);
        assert_eq!(
            msg,
    ...
    fn format_message_xml_with_responds_to() {
        let msg = format_session_message("web", "done", false, 113, Some(47));
