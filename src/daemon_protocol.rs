...
/// Notifications delivered to a session's agent (MCP subscriber).
#[derive(Clone, Debug)]
...
fn format_session_message(from: &str, message: &str, expects_reply: bool) -> String {
    if expects_reply {
...
impl DaemonState {
    ...
    pub fn apply(&mut self, event: Event) -> Vec<Effect> {
        match event {
            ...
            Event::Send {
                ...
                message,
                expects_reply,
            } => self.apply_send(&from, &to, &message, expects_reply),
        }
    }
    ...
    fn apply_incoming_wire(
        ...
        match msg {
            WireMessage::SessionSend {
                ...
                message,
                expects_reply,
            } => self.apply_incoming_send(&from, &to, &message, expects_reply),
        ...
        match target {
                ...
                if matches!(session.origin, Origin::Local) && session.metadata.networked =>
            {
                if let Some(ref pane) = session.pane {
                    let formatted = format_session_message(&display_from, message, expects_reply);
                    ...
                    if expects_reply && sender_known {
                        effects.push(Effect::NotifyAgent {
                            session_id: to.to_string(),
                            msg: AgentNotification::MessageDelivered {
                                from: from.to_string(),
                    ...
                    effects.push(Effect::NotifyAgent {
                        ...
                        msg: AgentNotification::ReplySent { to: to.to_string() },
                    });
    ...
    fn apply_send(
        ...
        match &session.origin {
            Origin::Local => {
                if let Some(ref pane) = session.pane {
                    let formatted = format_session_message(from, message, expects_reply);
                    effects.push(Effect::InjectMessage {
                        pane: pane.clone(),
                        message: formatted,
                        vim_mode: session.metadata.vim_mode,
                    ...
                    effects.push(Effect::NotifyAgent {
                        session_id: from.to_string(),
                        msg: AgentNotification::ReplySent {
            ...
            Origin::Remote(_) => {
                ...
                effects.push(Effect::NotifyAgent {
                    session_id: from.to_string(),
                    msg: AgentNotification::ReplySent { to: wire_to },
                });
                effects.push(Effect::LogMessage {
                    from: from.to_string(),
                    to: resolved_to.clone(),
