...
pub fn save_sessions(data_dir: &Path, sessions: &[PersistedSession]) -> Result<()> {
    ...
    atomic_write(&data_dir.join("sessions.json"), data.as_bytes())
}
...
pub fn load_settings(data_dir: &Path) -> Result<OuijaSettings> {
    ...
    if !path.exists() {
        return Ok(OuijaSettings::default());
    }
    let data = std::fs::read_to_string(&path)?;
    Ok(serde_json::from_str(&data)?)
}
...
pub fn save_settings(data_dir: &Path, settings: &OuijaSettings) -> Result<()> {
    ...
    atomic_write(&data_dir.join("settings.json"), data.as_bytes())
}
...
pub fn save_tasks(data_dir: &Path, tasks: &HashMap<String, ScheduledTask>) -> Result<()> {
    ...
    atomic_write(&data_dir.join("tasks.json"), data.as_bytes())
}
...
mod tests {
    ...
    fn connections_round_trip() {
        ...
        assert!(loaded[0].peer_name.is_none());
        assert_eq!(loaded[1].ticket, "ticket-def");
        assert_eq!(loaded[1].peer_name.as_deref(), Some("remote1"));
    }
    ...
    #[test]
    fn peer_name_backward_compat() {
        let dir = tempfile::tempdir().unwrap();
        // Simulate old format without peer_name field
