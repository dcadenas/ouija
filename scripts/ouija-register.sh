...
RESP=$(curl -sf -X POST "http://localhost:${OUIJA_PORT:-7880}/api/hooks/session-start" \
  ...
  -d "$(jq -cn --arg pane "$PANE" --arg cwd "$CWD" --arg backend_session_id "$BACKEND_SESSION_ID" --arg adapter "claude-code" --arg launch_session_id "${OUIJA_SESSION_ID:-}" --arg launch_credential "${OUIJA_SESSION_START_CREDENTIAL:-}" --arg incarnation "$INCARNATION" \
