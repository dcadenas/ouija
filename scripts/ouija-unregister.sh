...
curl -sf -X POST "http://localhost:${OUIJA_PORT:-7880}/api/hooks/session-end" \
  -H "Content-Type: application/json" -d "$BODY" >/dev/null 2>&1 &
disown
[ -n "$BACKEND_SESSION_ID" ] && "$SCRIPT_DIR/hook-incarnation.sh" delete "$BACKEND_SESSION_ID" "$INCARNATION"
exit 0
